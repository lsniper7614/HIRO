...
from models import ControllerActor, ControllerCritic, ManagerActor, ManagerCritic
...
class Controller(object):
    ...
    def train(self, replay_buffer, iterations,
        ...
        for it in range(iterations):
            ...
            next_sg = get_tensor(self.subgoal_transition(state, sg, next_state))

            # Q target = reward + discount * Q(next_state, pi(next_state))
            target_Q = self.critic_target(get_tensor(next_state), next_sg,
                                          self.actor_target(get_tensor(next_state), next_sg))
